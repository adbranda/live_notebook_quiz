  ...
  room.lastResult = {
    ...
    leaderboard: [...room.players.values()]
      ...
      .map((p, index) => ({
        ...
        name: p.name,
        score: p.score,
        pointsThisRound: p.pointsThisRound || 0,
      })),
  };
...
function sendCurrentState(socket, room, role, player = null) {
  if (role === "host") socket.emit("host:ready", publicRoom(room));
    ...
    if (room.state === "question")
      socket.emit("question:started", publicQuestion(room));
    else if (room.state === "results" && room.lastResult)
      socket.emit("question:ended", room.lastResult);
    ...
    const room = {
      ...
      settings: {
        timePerQuestion: Math.max(
          5,
          Math.min(120, Number(req.body?.timePerQuestion || 20)),
        ),
      },
      createdAt: Date.now(),
    };
...
app.get("/api/rooms/:pin/qr", async (req, res) => {
  ...
  res
    .type("png")
    .send(await QRCode.toBuffer(joinUrl, { width: 500, margin: 2 }));
});
...
io.on("connection", (socket) => {
  socket.on("host:claim", ({ pin, sessionId }) => {
    ...
    if (sessionId !== room.hostSessionId)
      return socket.emit(
        "error:message",
        "Host session is invalid or expired.",
      );
    if (room.hostSocket && room.hostSocket !== socket.id)
      return socket.emit("error:message", "This game already has a host.");
    room.hostSocket = socket.id;
    room.hostDisconnectedAt = null;
    sockets.set(socket.id, { pin: room.pin, role: "host" });
    sendCurrentState(socket, room, "host");
  });
  socket.on("player:join", ({ pin, name, sessionId }) => {
    ...
    if (room.state === "finished")
      return socket.emit("error:message", "This game has finished.");
    let existing = null;
    if (sessionId)
      existing = [...room.players.values()].find(
    ...
    if (existing) {
      if (existing.socketId && existing.socketId !== socket.id)
        return socket.emit(
          "error:message",
          "That player session is already connected.",
        );
      ...
      sendCurrentState(socket, room, "player", existing);
      broadcastRoom(room);
      return;
    }
    ...
    if (sameName)
      return socket.emit(
        ...
        sameName.socketId
          ...
          : "That nickname belongs to a recent session. Reopen this game on the same device to restore it.",
      );
    const player = {
  ...
  socket.on("host:start", () => {
    ...
    if (!room.players.size)
      return socket.emit("error:message", "At least one player must join.");
  ...
  socket.on("player:answer", ({ answer }) => {
    ...
    if (!room || membership.role !== "player" || room.state !== "question")
      return;
    const player = room.players.get(membership.playerId);
    if (!player || player.answered || !["A", "B", "C", "D"].includes(answer))
      return;
    ...
    if (room.hostSocket)
      io.to(room.hostSocket).emit("answer:count", {
        answered: [...room.players.values()].filter((p) => p.answered).length,
...
setInterval(() => {
  ...
  for (const [pin, room] of rooms) {
    for (const [id, player] of room.players) {
      if (player.disconnectedAt && now - player.disconnectedAt > SESSION_TTL_MS)
        room.players.delete(id);
    }
    if (
      room.hostDisconnectedAt &&
      now - room.hostDisconnectedAt > SESSION_TTL_MS
    ) {
      clearTimeout(room.timer);
      emitToRoom(room, "game:closed", { reason: "The host session expired." });
      rooms.delete(pin);
      continue;
    }
    if (
      !room.hostSocket &&
      room.state === "lobby" &&
      now - room.createdAt > SESSION_TTL_MS
    ) {
      rooms.delete(pin);
    }
  }
...
server.listen(PORT, () =>
  console.log(`Notebook Live Quiz running on http://localhost:${PORT}`),
);
