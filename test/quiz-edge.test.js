...
const good = row(['1', 'Question', 'Hint', 'A', 'B', 'C', 'D', 'C', 'Reason']);
...
{
  const quiz = parseQuizCsv(`${header}\n${row(['1', 'A, B?', 'Use the comma.', 'A', 'B', 'C', 'D', 'A', 'Because'])}`);
  validateQuiz(quiz);
  assert.equal(quiz.questions[0].question, 'A, B?');
}

{
  const quiz = parseQuizCsv(`${header}\n${row(['1', 'A "quoted" question', 'Hint', 'A', 'B', 'C', 'D', 'A', 'A "reason"'])}`);
  validateQuiz(quiz);
...
expectError('missing question', `${header}\n${row(['1', '', 'Hint', 'A', 'B', 'C', 'D', 'A', 'Reason'])}`, 'Question is empty');
expectError('missing hint', `${header}\n${row(['1', 'Q', '', 'A', 'B', 'C', 'D', 'A', 'Reason'])}`, 'Hint is empty');
expectError('missing rationale', `${header}\n${row(['1', 'Q', 'H', 'A', 'B', 'C', 'D', 'A', ''])}`, 'Rationale is empty');
expectError('missing option A', `${header}\n${row(['1', 'Q', 'H', '', 'B', 'C', 'D', 'A', 'R'])}`, 'Option A is empty');
expectError('missing option B', `${header}\n${row(['1', 'Q', 'H', 'A', '', 'C', 'D', 'A', 'R'])}`, 'Option B is empty');
expectError('missing option C', `${header}\n${row(['1', 'Q', 'H', 'A', 'B', '', 'D', 'A', 'R'])}`, 'Option C is empty');
expectError('missing option D', `${header}\n${row(['1', 'Q', 'H', 'A', 'B', 'C', '', 'A', 'R'])}`, 'Option D is empty');
expectError('bad answer', `${header}\n${row(['1', 'Q', 'H', 'A', 'B', 'C', 'D', 'E', 'R'])}`, 'Correct Answer must be A, B, C, or D');

{
  const rows = Array.from({ length: 20 }, (_, i) => row([String(i + 1), `Q${i}`, 'H', 'A', 'B', 'C', 'D', 'A', 'R']));
